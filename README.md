...
- **Deck Builder that shows your deck** — Leader + live deck stats up top (cost curve, power curve, counter stats, keywords, types, and turn-by-turn on-curve draw odds computed hypergeometrically from your actual list), the cards laid out as visual stacks below, card search under that. Click any stack to pick between all its artworks; copy the list to the clipboard (OPTCGSim-compatible `4xOP01-016` lines) or import one; name decks whatever you like.
- **Rip Packs + Collection** — open as many boosters as you like from any OP/EB set. Packs are generated server-side from that set's real card list (12 cards, 24 per box) using community-documented pull rates (≈1 Rare guaranteed, ≈8 SR / ≈12 L / ≈1 SEC / ≈2 alt-art per box, manga/TR/SP as long-shot chase — Bandai publishes no official rates; sources are listed in the in-app ⓘ and `server/packs.js`). The pack on screen is the real product photo of that set's booster (relayed from Bandai's product pages via `/api/set-image/:set`, with a foil design as fallback); it tears open with an animation, the cards come out as one face-down stack and are revealed one at a time — each press swipes the top card off to the right (Pokémon TCG Pocket style) — with a drum-roll build-up before the last card, rarity-scaled fanfares and confetti. Click any revealed card (packs, collection, results) for a big artwork viewer with ◀ ▶ browsing. Every pull is saved to your account's collection (guests too, for the life of the session).
- **Settings** (page + in-game ⚙️) — Auto Draw, Auto Skip Block, Auto Skip Trigger, Confirm DON!! Attach, Confirm End Turn, Dynamic Playsheets, Confirm Before Countering, Enable Attach All DON!!, plus music on/off and separate music / sound-effect volumes. Saved on the device.
- **Music & sound** — synthesized sound effects and an original nautical instrumental loop (both generated in the browser, nothing copyrighted shipped). Drop your own `public/audio/bgm.mp3` to use a real track instead.
