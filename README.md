...
- **Deck Builder** — browse and filter the full card pool (2,512 unique cards: every OP/EB/PRB booster and ST starter deck, no gaps) and build/save legal 50-card decks. Search by name, card ID, type, or text.
...
Card art is hotlinked from a public card database at render time — no card images are stored or redistributed by this app. Card backs and DON!! cards are drawn as SVGs (`public/img/cardback.svg`, `public/img/don.svg`); if you'd rather use real scans, drop `cardback.png` and/or `don.png` into `public/img/` and they'll be used automatically.
...
public/            All static frontend files (plain HTML/CSS/JS, no build step)
  ...
  data/cards.json   The full card database (generated — see data/parse_cards.py)
server/
  ...
  cards.js          Serves the card database
